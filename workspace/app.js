...
(function(Opal) {
  ...
  Opal.add_stubs(['$require', '$ready?', '$find', '$html=', '$get', '$[]', '$json', '$on', '$which', '$text=', '$sample']);
  self.$require("opal");
  self.$require("opal-jquery");
  return ($a = ($b = $scope.get('Document'))['$ready?'], $a.$$p = (TMP_1 = function(){var self = TMP_1.$$s || this, $a, $b, TMP_2, $c, TMP_3, title = nil, body = nil, html = nil, lookup = nil, keypressed = nil, twit = nil, twits = nil;

  title = $scope.get('Element').$find("#title");
    (($a = ["<h1>ASCII Character Lookup Tool</h1>"]), $b = title, $b['$html='].apply($b, $a), $a[$a.length-1]);
    body = $scope.get('Element').$find("#body");
    (($a = ["<p>Type any character and the code will be shown below</p>"]), $b = body, $b['$html='].apply($b, $a), $a[$a.length-1]);
    html = $scope.get('Element').$find("html");
    lookup = $scope.get('Element').$find("#lookup");
    keypressed = $scope.get('Element').$find("#keypressed");
    twit = $scope.get('Element').$find("#twit");
    twits = "";
    ($a = ($b = $scope.get('HTTP')).$get, $a.$$p = (TMP_2 = function(res){var self = TMP_2.$$s || this;
if (res == null) res = nil;
    return twits = res.$json()['$[]']("twitsCollection")}, TMP_2.$$s = self, TMP_2), $a).call($b, "https://jinthepimp.herokuapp.com/api/twits");
    return ($a = ($c = html).$on, $a.$$p = (TMP_3 = function(evt){var self = TMP_3.$$s || this, $a, $b, chrcode = nil, key = nil;
if (evt == null) evt = nil;
    chrcode = evt.$which();
      key = String.fromCharCode(chrcode);
      (($a = [key]), $b = keypressed, $b['$text='].apply($b, $a), $a[$a.length-1]);
      (($a = [chrcode]), $b = lookup, $b['$text='].apply($b, $a), $a[$a.length-1]);
      return (($a = [twits.$sample()]), $b = twit, $b['$text='].apply($b, $a), $a[$a.length-1]);}, TMP_3.$$s = self, TMP_3), $a).call($c, "keydown");}, TMP_1.$$s = self, TMP_1), $a).call($b);
})(Opal);
